use bevy::{prelude::{Vec2, Vec3}, sprite::collide_aabb::Collision};
...
pub fn velocity_collision (
    ...
    if a_new_min.x < b_new_max.x && a_new_max.x > b_new_min.x && a_new_min.y < b_new_max.y && a_new_max.y > b_new_min.y {

        if a_velocity.x - b_velocity.x < 0.0 && a_new_min.x < b_new_max.x && a_min.x > b_max.x {

            Some((Collision::Right, b_new_max.x - a_new_min.x))

        } else if a_velocity.x - b_velocity.x > 0.0 && a_new_max.x > b_new_min.x && a_max.x < b_min.x {

            Some((Collision::Left, a_new_max.x - b_new_min.x))

        } else if a_velocity.y - b_velocity.y < 0.0 && a_new_min.y < b_new_max.y && a_min.y > b_max.y {

            Some((Collision::Top, b_new_max.y - a_new_min.y))

        } else if a_velocity.y - b_velocity.y > 0.0 && a_new_max.y > b_new_min.y && a_max.y < b_min.y {

            Some((Collision::Bottom, a_new_max.y - b_new_min.y))
